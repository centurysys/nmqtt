...
proc handleClient(client: AsyncSocket, connectionNumber: int) {.async.} =
  try:
    ...
    if connectionNumber <= 2:
      # Reject the first two connections. The client should apply the same
      # reconnect policy used for TCP/TLS setup failures: 1 second, then 2.
      await client.send("\x20\x02\x00\x05")
      await sleepAsync(100)
      return

    # Third connection succeeds so the test can verify policy reset and the
...
suite "CONNACK rejection backoff regression":
  test "applies reconnect backoff after rejected CONNACK":
    proc run() {.async.} =
      let broker = newTestBroker()
      defer:
        broker.stop()

      let ctx = newMqttCtx("nmqttConnAckBackoffRegression")
      ...
      # Leave some scheduler tolerance while still proving that the old
      # roughly-one-second retry loop is no longer used for the second reject.
