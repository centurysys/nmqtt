...
proc handleClient(client: AsyncSocket, connectionNumber: int) {.async.} =
  try:
    ...
    if connectionNumber == 1:
      # MQTT 3.1.1 CONNACK: connection refused, not authorized (0x05).
      # A rejected CONNACK must never transition the client to Connected.
      await client.send("\x20\x02\x00\x05")
      await sleepAsync(100)
      return

    # Second connection succeeds so the test also verifies that automatic
    # reconnect continues after the rejected CONNACK.
...
suite "CONNACK rejection regression":
  test "does not report a rejected CONNACK as connected":
    proc run() {.async.} =
      let broker = newTestBroker()
      defer:
        broker.stop()

      let ctx = newMqttCtx("nmqttConnAckRejectRegression")
