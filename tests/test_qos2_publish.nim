...
type
  TestBroker = ref object
    ...
    port: Port
    running: bool
    dropFirstPubRel: bool
    connectionCount: int
    publishCount: int
    pubRelCount: int
    publishMsgId: uint16
    firstPubRelMsgId: uint16
    resentPubRelMsgId: uint16
...
proc packetMsgId(body: string): uint16 =
  if body.len < 2:
    raise newException(ValueError, "MQTT packet has no message ID")
...
#
# ------------------------------------------------------------------------------
proc sendPubRec(client: AsyncSocket, msgId: uint16) {.async.} =
  var pkt = newString(4)
  pkt[0] = 0x50.char
  pkt[1] = 0x02.char
  pkt[2] = (msgId shr 8).uint8.char
  pkt[3] = (msgId and 0xff).uint8.char
  await client.send(pkt)

# ------------------------------------------------------------------------------
#
# ------------------------------------------------------------------------------
proc sendPubComp(client: AsyncSocket, msgId: uint16) {.async.} =
  var pkt = newString(4)
  pkt[0] = 0x70.char
  pkt[1] = 0x02.char
  pkt[2] = (msgId shr 8).uint8.char
  ...
  try:
    ...
    while true:
      ...
      of 3:
        ...
        broker.publishMsgId = msgId
        await client.sendPubRec(msgId)

      of 6:
        let msgId = packetMsgId(body)
        broker.pubRelCount.inc()

        if broker.pubRelCount == 1:
          broker.firstPubRelMsgId = msgId
        else:
          broker.resentPubRelMsgId = msgId

        if broker.dropFirstPubRel and connectionNumber == 1:
          # PUBREC was already sent, so the client must resume this QoS 2
          # transaction from PUBREL after reconnect rather than resend PUBLISH.
          return

        await client.sendPubComp(msgId)
...
#
# ------------------------------------------------------------------------------
proc newTestBroker(dropFirstPubRel = false): TestBroker =
  ...
  result = TestBroker(
    ...
    running: true,
    dropFirstPubRel: dropFirstPubRel,
  )
...
suite "QoS 2 publish regression":
  test "completes the outgoing QoS 2 handshake":
    proc run() {.async.} =
      let broker = newTestBroker()
      defer:
        broker.stop()

      let ctx = newMqttCtx("nmqttQos2PublishRegression")
      ...
      check await waitUntil(proc(): bool = broker.pubRelCount >= 1)
      check broker.publishCount == 1
      check broker.publishMsgId != 0
      check broker.firstPubRelMsgId == broker.publishMsgId
  ...
  test "resumes from PUBREL after reconnect":
    proc run() {.async.} =
      let broker = newTestBroker(dropFirstPubRel = true)
      defer:
        broker.stop()

      let ctx = newMqttCtx("nmqttQos2ReconnectRegression")
      ctx.setHost("127.0.0.1", broker.port.int)
      ...
      let published = await ctx.publishId(
        topic = "test/qos2/reconnect",
        message = "payload",
        qos = 2,
      )

      check published.isSome
      check await waitUntil(proc(): bool = broker.pubRelCount >= 2)
      check broker.connectionCount >= 2
      check broker.publishCount == 1
      check broker.publishMsgId != 0
      check broker.firstPubRelMsgId == broker.publishMsgId
      check broker.resentPubRelMsgId == broker.publishMsgId
      check await waitUntil(proc(): bool = ctx.msgQueue() == 0)
      check ctx.isConnected()
